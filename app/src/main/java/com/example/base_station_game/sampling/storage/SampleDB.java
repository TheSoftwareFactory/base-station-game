package com.example.base_station_game.sampling.storage;
...
//                Logger.d(TAG, "Now: " + now + " Moment ago: " + momentAgo);
                ...
                for (Sample sample : sampleMap.values()) {
                    long timestamp = (long) sample.getTimestamp() * 1000; // Database uses seconds
...
//                        Logger.d(TAG, "Smaller than " + momentAgo + " returning!");
    ...
    private Sample fillSample(Cursor cursor) {
        ...
        if (sampleB != null) {
            ByteArrayInputStream bis = new ByteArrayInputStream(sampleB);
            ObjectInput in = null;
            try {
...
//            Logger.d("SampleDB.addSample()", "The sample's battery level=" + s.getBatteryLevel());
        ...
        try {
            ...
            initialValues.put(COLUMN_SAMPLE, data);
        } catch (IOException e) {
            e.printStackTrace();
        } finally {
            try {
